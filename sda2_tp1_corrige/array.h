 ...
 */
static inline void array_swap_elements(long int* const array, size_t i, size_t j)
{
...
/**
 * \brief Copy elements from an array into another array.
 * \param[in,out] dest Destination array.
 * \param[in]     src  Source array.
 * \param[in]     size Size of the source array.
 */
static inline void array_copy_elements(
    long int* const src, long int* const dest, size_t size)
{
  for (size_t i = 0; i < size; ++i)
    dest[i] = src[i];
}
